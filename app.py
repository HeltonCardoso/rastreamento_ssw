...
def ler_google_sheets():
    ...
    try:
        ...
        if 'spreadsheets/d/' in GOOGLE_SHEETS_URL:
            ...
            spreadsheet_id = match.group(1) if match else GOOGLE_SHEETS_URL
        else:
        ...
        if not aba and worksheets:
            ...
            print(f"⚠️ Nenhuma aba com 'SSW' encontrada. Usando: {aba}")
        ...
        raise Exception("Execute: pip install gspread google-auth")
    except FileNotFoundError:
        raise Exception(f"Credenciais não encontradas: {GOOGLE_CREDENTIALS_PATH}")
    except Exception as e:
...
class Processo:
    def __init__(self, pid):
        ...
        self.mensagem = ""
        self.stats    = {
            'entregues': 0, 'pendentes': 0, 'alertas': 0,
            'atrasados': 0, 'devolvidos': 0, 'erros': 0
        }
        self.ultimos   = []
    ...
    def atualizar_stats(self, status: str):
        """Atualiza contadores de status (thread-safe)."""
        with self._lock:
            if 'ENTREGUE' in status:
                self.stats['entregues'] += 1
            elif 'ATRASADO' in status:
                self.stats['atrasados'] += 1
            elif 'PREVISÃO' in status:
                self.stats['alertas'] += 1
            elif 'DEVOLVIDO' in status:
                self.stats['devolvidos'] += 1
            elif 'ERRO' in status:
                self.stats['erros'] += 1
            else:
                self.stats['pendentes'] += 1


def executar(pid, df, coluna_xml, workers):
    """Executa o processamento paralelo em thread de background."""
    p = processos[pid]
    p.status = "processando"
    p.total  = len(df)
    p.enviar('inicio', {'total': p.total})

    # ----- callback chamado a cada pedido concluído -----
    def on_progresso(atual, total, res):
        if p.status == "cancelado":
            return

        status = res.get('status', '')
        p.atualizar_stats(status)

        with p._lock:
            p.progresso = atual
            p.mensagem  = f"Processando {atual}/{total}"
            ...
            p.ultimos.append(ultimo)
            if len(p.ultimos) > 20:
                p.ultimos.pop(0)
            stats_snapshot = dict(p.stats)

        p.enviar('atualizacao', {
            'progresso': atual,
            'total':     total,
    ...
    processador = ProcessadorSSW(workers=workers)

    # Wrapper para suporte a cancelamento: encerra o executor se cancelado
    resultados_df = processador.processar_lote(
        df,
...
def index():
    return render_template(
        'index.html',
        google_sheets_url=GOOGLE_SHEETS_URL if GOOGLE_CONFIGURADO else "Não configurado",
        workers_padrao=WORKERS_PADRAO
    )
...
def processar():
    ...
    workers    = int(request.form.get('workers', WORKERS_PADRAO))

    # Garante limites seguros
    workers = max(1, min(workers, 20))

    try:
        print(f"📥 Processando fonte: {fonte} | workers: {workers}")
        ...
        thread = threading.Thread(target=executar, args=(pid, df, coluna_xml, workers))
        thread.daemon = True
        thread.start()
